...
def merge_predictions(hotspots_df):
    """
    Merge predictions.csv (output of predictive_model.py) onto the main
    hot-spot table by SUBURB. This is OPTIONAL — the map still builds fine
    without it, just without the 4th "Predicted Next Month" popup section
    and without the Rising Risk toggle having anything to show.

    Only suburbs that appear in BOTH hotspots_with_saps.csv AND
    predictions.csv get a forecast — this is expected and correct:
    predictive_model.py only forecasts suburbs with enough historical
    months of data (see MIN_MONTHS_HISTORY in that script), so not every
    hot-spot suburb will have a qualifying forecast.
    """
    if not os.path.exists(PREDICTIONS_CSV):
        print(f"\nNote: {PREDICTIONS_CSV} not found — skipping prediction layer. "
              f"Run predictive_model.py first if you want forecasts shown on the map.")
        hotspots_df = hotspots_df.copy()
        hotspots_df["has_prediction"] = False
        return hotspots_df

    predictions = pd.read_csv(PREDICTIONS_CSV)
    print(f"\nLoaded {len(predictions)} suburb forecasts from {PREDICTIONS_CSV}.")

    keep_cols = ["SUBURB", "forecast_month", "predicted_incidents_next_month",
                 "trend_direction", "trend_slope", "months_of_history_used"]
    predictions_subset = predictions[keep_cols].copy()

    merged = hotspots_df.merge(predictions_subset, on="SUBURB", how="left")
    merged["has_prediction"] = merged["predicted_incidents_next_month"].notna()

    n_with_forecast = merged["has_prediction"].sum()
    n_increasing = (merged["trend_direction"] == "Increasing").sum()
    print(f"  -> {n_with_forecast}/{len(merged)} hot-spot suburbs have a forecast "
          f"({n_increasing} flagged 'Increasing')")
...
def build_popup_html(row):
    ...
    # --- Prediction section (only if this suburb has a qualifying forecast) ---
    # Distinctly colored (purple) so it's immediately visually separate from
    # the blue (Discovery, historical) and green (SAPS, historical) sections
    # above -- this section is explicitly FORWARD-LOOKING, not historical,
    # and that distinction matters enough to be visually obvious, not just
    # stated in text.
    if row.get("has_prediction", False):
        trend = row.get("trend_direction", "")
        trend_arrow = {"Increasing": "&#9650;", "Decreasing": "&#9660;", "Stable": "&#9679;"}.get(trend, "")
        trend_color = {"Increasing": "#c0392b", "Decreasing": "#1E7B34", "Stable": "#666666"}.get(trend, "#666666")

        prediction_section = (
            f"<div style='border-left:4px solid #6C3483; padding-left:8px; margin-top:6px;'>"
            f"<div style='color:#6C3483; font-weight:bold; font-size:12px; text-transform:uppercase; margin-bottom:3px;'>"
            f"Predicted Next Month ({row.get('forecast_month', 'N/A')})</div>"
            f"Forecasted incidents: {row['predicted_incidents_next_month']:.1f}<br>"
            f"Trend: <span style='color:{trend_color}; font-weight:bold;'>{trend_arrow} {trend}</span><br>"
            f"<span style='color:#999; font-size:10px;'>Based on {int(row.get('months_of_history_used', 0))} "
            f"months of history &mdash; backtested forecast, not a guarantee</span>"
            f"</div>"
        )
    else:
        prediction_section = (
            f"<div style='border-left:4px solid #CCCCCC; padding-left:8px; margin-top:6px; color:#999; font-size:11px;'>"
            f"Not enough historical data yet for a reliable forecast."
            f"</div>"
        )
...
def build_map_html(geocoded_df, output_path, saps_only_geocoded=None):
    ...
    """
    markers_js = []
    marker_refs_js = []  # tracks each marker's JS variable name + its trend, for the toggle
    marker_counter = 0
    ...
    for _, row in geocoded_df.iterrows():
        ...
        # Assign each marker a unique JS variable name so the Rising Risk
        # toggle can show/hide it individually via Leaflet's addTo/remove.
        var_name = f"m{marker_counter}"
        marker_counter += 1
        trend = row.get("trend_direction", "") if row.get("has_prediction", False) else "NoData"

        markers_js.append(
            f"var {var_name} = L.circleMarker([{row['lat']}, {row['lon']}], {{"
            ...
            f"allMarkers.push({{marker: {var_name}, trend: '{trend}'}});"
        )
    ...
    if saps_only_geocoded is not None and len(saps_only_geocoded) > 0:
        ...
        for _, row in saps_only_geocoded.iterrows():
            ...
            markers_js.append(
                f"var {var_name} = L.marker([{row['lat']}, {row['lon']}], {{"
                f"icon: L.divIcon({{"
                f"className: '', "
                ...
                f"}})"
                f"}}).bindPopup('{popup_html}');"
                # SAPS-only precincts have no Discovery-based forecast (the
                # predictive model trains on Discovery claim dates), so
                # they're tagged 'NoData' -- shown in "All" mode, hidden in
                # "Rising Risk Only" mode, same as Discovery suburbs with
                # no qualifying forecast.
                f"allMarkers.push({{marker: {var_name}, trend: 'NoData'}});"
            )
...
<head>
    ...
    <style>
        ...
        #view-toggle button.active {{ background: #6C3483; color: white; }}
        #view-toggle button:not(.active) {{ background: #eee; color: #333; }}
        #rising-risk-count {{
...
<body>
    ...
    <div id="view-toggle">
        <button id="btn-all" class="active" onclick="showAllMarkers()">All Hot-Spots</button>
        <button id="btn-rising" onclick="showRisingRiskOnly()">&#9650; Rising Risk Only</button>
        <div id="rising-risk-count"></div>
    </div>
    ...
    <div id="legend">
        ...
        {saps_only_legend}
        <div class="legend-divider"></div>
        <h4>Predicted Next Month (popup section)</h4>
        <div style="font-size:11px; color:#666;">Purple popup section shows a backtested forecast where enough history exists. Use the "Rising Risk Only" button above to filter the map to suburbs flagged Increasing.</div>
    </div>
    ...
    <script>
        ...
        // Every marker is pushed here as it's created, tagged with its
        // trend_direction ('Increasing', 'Decreasing', 'Stable', or
        // 'NoData' for markers with no qualifying forecast). This array
        // is what the Rising Risk toggle filters against.
        var allMarkers = [];

        {markers_block}

        // Default view: show everything (matches original map behavior)
        allMarkers.forEach(function(m) {{ m.marker.addTo(map); }});
        ...
        function showAllMarkers() {{
            ...
            document.getElementById('rising-risk-count').innerText = '';
        }}

        function showRisingRiskOnly() {{
            var risingCount = 0;
            allMarkers.forEach(function(m) {{
            ...
            document.getElementById('btn-rising').classList.add('active');
            document.getElementById('btn-all').classList.remove('active');
            document.getElementById('rising-risk-count').innerText =
                risingCount + ' suburb(s) flagged Increasing';
        }}
    </script>
</body>
</html>
...
def main():
    ...
    hotspots = merge_predictions(hotspots)
